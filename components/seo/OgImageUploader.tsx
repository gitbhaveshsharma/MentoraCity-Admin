...
export function OgImageUploader({ centerId, branchId, value, onChange }: { centerId: string; branchId: string; value?: string; onChange: (url: string) => void }) {
  ...
  return <div><div style={{ display: "flex", gap: 10, alignItems: "center" }}>{value && <img src={value} alt="OG preview" style={{ width: 84, height: 44, borderRadius: 5, objectFit: "cover", border: "1px solid #e8ebef" }} />}<button type="button" className="btn btn-ghost btn-sm" onClick={() => input.current?.click()} disabled={busy}>{busy ? "Uploading…" : "Upload image"}</button><input ref={input} type="file" hidden accept=".jpg,.jpeg,.png,.webp" onChange={(event) => event.target.files?.[0] && upload(event.target.files[0])} /></div>{dimensions && <small className="counter invalid">Selected size {dimensions}; 1200×630px is recommended.</small>}</div>;
}
